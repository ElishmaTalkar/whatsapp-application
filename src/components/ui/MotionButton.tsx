...
const MotionButton: FC<Props> = ({ label, classes }) => {
    return (
        ...
        >
            <span
                className='absolute left-0 top-1/2 -translate-y-1/2 m-0 block h-12 w-12 bg-[#5b6af5] rounded-full duration-500 ease-in-out group-hover:w-full'
                aria-hidden='true'
            ></span>
            ...
            <span className='relative block py-3 pl-12 pr-6 text-[#f2f4ff] font-bold text-lg tracking-tight whitespace-nowrap duration-500 group-hover:text-white text-left'>
                {label}
            </span>
        </button>
    )
